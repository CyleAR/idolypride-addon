...
namespace StringParser {
    ...
    ParseItems ParseItems::parse(const std::string &str, bool parseTags) {
        ...
        std::u16string origText = HoshimiLocal::Misc::ToUTF16(str);
        bool isInTag = false;
            ...
            if (parseTags && currChar == u'<') {
                if (!currentCacheText.empty()) {
                    result.items.push_back({isInFlagSequence ? ParseItemType::FLAG : ParseItemType::TEXT,
                                            HoshimiLocal::Misc::ToUTF8(currentCacheText)});
                ...
                isInTag = false;
                currentCacheText.push_back(currChar);
                result.items.push_back({ParseItemType::FLAG, HoshimiLocal::Misc::ToUTF8(currentCacheText)});
                currentCacheText.clear();
            } else if (isInTag) {
                currentCacheText.push_back(currChar);
            } else if (splitFlags.contains(currChar)) {
                if (!isInFlagSequence && !currentCacheText.empty()) {
                    result.items.push_back({ParseItemType::TEXT, HoshimiLocal::Misc::ToUTF8(currentCacheText)});
                    currentCacheText.clear();
                }
    ...
    std::string ParseItems::MergeText(ParseItems &textTarget, ParseItems &valueTarget) {
        ...
        const std::string ret = HoshimiLocal::Misc::StringFormat::stringFormatString(fmtText, values);
    ...
    std::string ParseItems::MergeText(const std::string &newStr) {
        ...
        return HoshimiLocal::Misc::StringFormat::stringFormatString(newStr, values);
    }
