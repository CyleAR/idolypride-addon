...
#define WINDOWS_MODE 0 // Â¶ÇÊûú?ÄË¶ÅË??π‰∏∫ 1 | 1 if you need
...
public:
	...
	struct Class final {
		...
		/**
		 * \brief ?∑ÂèñÁ±ªÊ??âÂÆû‰æ?
		 * \tparam T ËøîÂõû?∞ÁªÑÁ±ªÂûã
		 * \param type Á±?
		 * \return ËøîÂõûÂÆû‰æã?áÈíà?∞ÁªÑ
		 */
		template <typename T>
	...
	/**
	 * \brief Ë∞ÉÁî®dll?ΩÊï∞
	 * \tparam Return ËøîÂõûÁ±ªÂûã (ÂøÖÈ°ª)
	 * \tparam Args ?ÇÊï∞Á±ªÂûã (??ª•ÂøΩÁï•)
	 * \param funcName dllÂØºÂá∫?ΩÊï∞?çÁß∞
	 * \param args ?ÇÊï∞
	 * \return Ê®°ÊùøÁ±ªÂûã
	 */
	template <typename Return, typename... Args>
	static auto Invoke(const std::string& funcName, Args... args) -> Return {
		...
		// Ê£Ä?•ÂáΩ?∞ÊòØ??∑≤ÁªèËé∑?ñÂú∞?Ä, Ê≤°Êúâ?ôËá™?®Ëé∑??
#if WINDOWS_MODE
